...
| 1.0.18 | 32 | リリース済み | チェックイン Universal Links・対戦表生成と大会ステータス・公式向けマイ大会/閲覧・収支修正・試合準備中表記等 |
...
一覧の判定は `lib/utils/tournament_status.dart` の `normalizeTournamentStatus` で旧表記を正規化したあとの `status` を前提とする（`エントリー締め切`→`エントリー締切`、`試合準備中`/`試合準備`→`大会準備中`）。コードとの差分があれば `lib/screens/tournament/tournament_search_screen.dart` の `_buildTournamentList` を正とし、本表を追従する。
...
（○＝フォロー中／みんな・種目・エリア・日付・検索などの条件も満たせば一覧に出る、×＝大会一覧から除外）

**フォロー中／みんな**: フォロー中＝フォローしている主催者＋自分／みんな＝まだフォローしていない主催者（発見用）。メンバー募集サブタブは大会 `status` では切らない。

#### マイ大会

| 項目 | 一般 | 公式 |
|------|------|------|
| 載る大会 | 主催 **または** エントリー参加 | **主催のみ** |
| これから／これまで | `終了`→これまで／それ以外→これから（正規化後の `status`） | 同じ |

#### 大会詳細（一般 vs 公式の差）

| 内容 | 一般 | 公式 |
|------|------|------|
| フォロー案内・下部ボタン | フォロー前提の文言・操作 | 閲覧しやすい文言・未フォローでも募集/エントリー導線、FollowService 連動 |
...
## アプリ化 進捗（2026/05/12 更新）

### リリース状況
- **iOS**: 🟢 1.0.18 リリース済み（チェックイン Universal Links・対戦表とステータス・公式向け表示・収支等）
