...
| 1.0.23 | 37 | 欠番（未提出） | チェックインQR修正は 1.0.24 に同梱して提出 |
| 1.0.24 | 38 | 審査提出準備 | 友達紹介リンクのアプリ直接起動・自動フォロー対応、チェックインQR「画像で保存」を写真ライブラリへ直接保存するよう修正 |
...
### リリース状況
- **iOS**: 🟢 1.0.22 リリース済み（公式向け閲覧・編集強化、チェックインQR共有修正）
