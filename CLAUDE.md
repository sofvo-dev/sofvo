...
| 1.0.28 | 42 | 審査提出準備 | QRチェックイン不具合修正（エントリー済みでも「対象データなし」で弾かれる問題・leaderUid照合追加・自動検出できない場合のチーム選択シート追加） |
...
### リリース状況
- **iOS**: 🟢 1.0.27 リリース済み（新規インストール時の紹介コード引き継ぎ・Android App Links 有効化）
