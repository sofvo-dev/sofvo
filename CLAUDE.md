...
| 1.0.29 | 43 | 審査提出準備 | QRチェックイン不具合修正（エントリー済みでも「対象データなし」・leaderUid照合追加・チーム選択シート）・順位表のチーム名タップでチーム対戦結果表示（セット別得点）・4チームリーグの決勝/3位決定戦の審判割り当て修正 |
...
### リリース状況
- **iOS**: 🟢 1.0.28 リリース済み（大会終了後のスコア閲覧・感想投稿再通知）
