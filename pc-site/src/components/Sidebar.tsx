...
export default function Sidebar() {
  ...
  const isActive = (href: string) => {
    ...
    if (href === "/tournaments" && pathname === "/tournaments/create") return false;
    if (href === "/tournaments" && pathname.startsWith("/tournaments/manage")) return false;
  ...
  const renderItem = (item: NavItem) => {
    ...
    return (
      <Link key={item.href} href={item.href}
        ...
        <NavIcon icon={item.icon} className={`w-5 h-5 flex-shrink-0 ${active ? "text-accent" : ""}`} />
