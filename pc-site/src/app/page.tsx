...
const BADGE_DEFS = [
  { id: "first_tournament", name: "初参加", icon: "🏐", threshold: 1, field: "tournamentsPlayed" as const },
  { id: "5_tournaments", name: "5大会参加", icon: "🎯", threshold: 5, field: "tournamentsPlayed" as const },
  { id: "10_tournaments", name: "10大会参加", icon: "🔥", threshold: 10, field: "tournamentsPlayed" as const },
  { id: "first_champion", name: "初優勝", icon: "🏆", threshold: 1, field: "championships" as const },
  { id: "3_champions", name: "3回優勝", icon: "👑", threshold: 3, field: "championships" as const },
  { id: "100_pts", name: "100Pt達成", icon: "💯", threshold: 100, field: "totalPoints" as const },
  { id: "500_pts", name: "500Pt達成", icon: "⭐", threshold: 500, field: "totalPoints" as const },
  { id: "1000_pts", name: "1000Pt達成", icon: "🌟", threshold: 1000, field: "totalPoints" as const },
  { id: "5_gadgets", name: "ガジェット5個", icon: "🎒", threshold: 5, field: "gadgetCount" as const },
  { id: "10_followers", name: "フォロワー10", icon: "👥", threshold: 10, field: "followersCount" as const },
];
...
export default function HomePage() {
  ...
  const myTournaments = user
    ? recentTournaments.filter((t) => t.organizerId === user.uid && t.status === "終了").slice(0, 6)
    : [];
  ...
  return (
    <div className="p-6 md:p-8 max-w-[1200px] mx-auto animate-fade-in">
        ...
        <>
          <div className="bg-white rounded-xl border border-gray-200 overflow-hidden mb-6">
            ...
            <div className="bg-white px-8 py-3 -mt-8 relative z-10 mx-6 rounded-xl border border-gray-200 shadow-sm flex items-center">
              <div className="flex items-center gap-5 flex-1">
                ...
                <div className="w-px h-6 bg-gray-200" />
                <Link href={`/profile/${user.uid}?tab=followers`} className="text-center hover:opacity-70 transition-opacity">
                  <div className="text-lg font-bold text-foreground">{profile.followersCount ?? 0}</div>
                  <div className="text-[11px] text-muted">フォロワー</div>
                </Link>
                <div className="w-px h-6 bg-gray-200" />
                <Link href={`/profile/${user.uid}?tab=following`} className="text-center hover:opacity-70 transition-opacity">
                  <div className="text-lg font-bold text-foreground">{profile.followingCount ?? 0}</div>
                  <div className="text-[11px] text-muted">フォロー中</div>
                </Link>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                <h2 className="text-sm font-bold text-foreground flex items-center gap-2">
                  <svg className="w-4 h-4 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21a3.745 3.745 0 01-3.068-1.593 3.746 3.746 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 013 12" /></svg>
                  バッジコレクション
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100">
                <h2 className="text-sm font-bold text-foreground flex items-center gap-2">
                  <svg className="w-4 h-4 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M20.25 14.15v4.25c0 1.094-.787 2.036-1.872 2.18-2.087.277-4.216.42-6.378.42s-4.291-.143-6.378-.42c-1.085-.144-1.872-1.086-1.872-2.18v-4.25" /></svg>
                  マイガジェット
                </h2>
          ...
          {myTournaments.length > 0 && (
            <section className="mb-6">
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-bold text-foreground flex items-center gap-2">
                  <svg className="w-4 h-4 text-primary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 18.75h-9m9 0a3 3 0 013 3h-15a3 3 0 013-3m9 0v-3.375c0-.621-.503-1.125-1.125-1.125h-.871M7.5 18.75v-3.375c0-.621.504-1.125 1.125-1.125h.872" /></svg>
                  大会結果
                </h2>
      ...
      {finishedTournaments.length > 0 && (
        <section className="mb-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-bold text-foreground">最近の大会</h2>
            <Link href="/tournaments" className="text-xs text-primary font-medium hover:underline">すべて見る</Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
