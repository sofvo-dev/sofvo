...
### さがすタブ UI改善
- 日付ボックスの色を月ごとに変更（1月=赤, 2月=ピンク, 3月=緑, 4月=紫, 5月=青...）
- 種別バッジを日付の下に分離配置（メンズ=青, レディース=ピンク, 混合=緑）
- フィルター（種別・エリア・日付）を折りたたみ式に変更、検索バー横のアイコンで展開
- フィルターアクティブ時に赤ドットインジケーター表示
- デフォルト表示を「募集中」のみに変更（開催中・決勝中・準備中は非表示）
- フィルター内に「終了した大会も表示」チェックボックス追加
- 4人制表記を非表示（ソフトバレーは必ず4人制のため）
- コート数を非表示
- 締切日をカードに追加
- 曜日を日付ボックスに追加
- 大会日が近い順にソート
- みんなの大会タブではメンバー募集トグルを非表示
- トグル名を「大会をさがす」「メンバーをさがす」に変更
- フィルターリセットボタン追加
