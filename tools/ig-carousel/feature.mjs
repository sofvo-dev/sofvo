...
function coverSlide(d){return `<div class="slide ${d.bg}">${wm(d.bg)}
  <div class="cover"><div class="pill">${d.pill}</div><div class="ctitle">${d.title}</div><div class="csub">${d.sub}</div></div>${footer}</div>`;}
function shotSlide(d){return `<div class="slide ${d.bg}">${wm(d.bg)}
  <div class="shot"><div class="shead">${d.head}</div>
    <div class="fphone"><img src="data:image/jpeg;base64,${img64(d.img)}"></div>
    <div class="scap">${d.cap}</div></div>${footer}</div>`;}
 ...
 *{margin:0;box-sizing:border-box}
 ...
 .footer{position:absolute;left:0;right:0;bottom:0;height:92px;background:${DARK};color:#fff;display:flex;align-items:center;justify-content:space-between;padding:0 48px}
 .footer .ftxt{font-size:32px;font-weight:700}.footer span{display:flex;align-items:center}
 .cover{position:absolute;top:0;left:0;right:0;bottom:92px;display:flex;flex-direction:column;justify-content:center;padding:100px 80px}
 ...
 .ctitle{font-family:'Dela';font-size:84px;line-height:1.34}.csub{margin-top:40px;font-size:42px;font-weight:700;color:${GOLD}}
 .shot{position:absolute;top:150px;left:0;right:0;bottom:92px;display:flex;flex-direction:column;align-items:center}
 .shead{font-family:'Dela';font-size:62px;text-align:center;padding:0 50px;margin-bottom:26px}
 .fphone{width:404px;height:760px;border:11px solid ${DARK};border-radius:48px;overflow:hidden;box-shadow:0 24px 50px rgba(0,0,0,.3);background:#000}
 .fphone img{width:100%;height:100%;object-fit:cover;object-position:top;display:block}
 .scap{margin-top:26px;font-size:40px;font-weight:700;text-align:center;padding:0 60px}
 .scap b{color:${GOLD}}
...
// 運営者向け 機能カルーセル
const slides=[
 {type:'cover',bg:'navy',pill:'大会運営',title:'大会運営、<br>ぜんぶアプリで。',sub:'紙も電卓も、もういらない'},
 {type:'shot',bg:'cream',head:'対戦表を自動作成',img:'app-bracket.jpg',cap:'チーム数を入れるだけ'},
 {type:'shot',bg:'navy',head:'リアルタイムでスコア',img:'app-score.jpg',cap:'入力すれば<b>全員に即共有</b>'},
 {type:'shot',bg:'cream',head:'順位表は自動集計',img:'app-ranking.jpg',cap:'勝敗・得失点も自動で'},
 {type:'shot',bg:'navy',head:'QRで受付',img:'app-checkin.jpg',cap:'かざすだけでチェックイン'},
